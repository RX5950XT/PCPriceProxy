...
## 專案現況
PCPriceProxy 整合原價屋、欣亞、Autobuy 三大通路的電腦零件價格，支援 MatchGroup 跨店整合比價卡片，並提供左側多級展開摺疊選單樹。第三種使用方式是 **Agent CLI / `/api/v1/agent`**（`pcprice search`），給語言模型精簡 JSON，不是 MCP。進專案的 Agent 先載 `skills/pcprice/SKILL.md`；契約見 `docs/agent-cli.md`。
...
### 計數單位（強制對齊）
- **UI 唯一單位＝`match_groups`（比價組）**：頂欄總量、側欄分類／子分類／品牌、列表「共 N 組」全部查同一張表。
- **`products` 列數不可當 UI 總量**：同 SKU 跨店多列（例：螢幕 1,271 件 → 1,191 組）。
- `/health`：`totalMatchGroups`＝UI 總量；`totalProducts`／來源 `productCount`＝爬取列，僅供運維。

### 第三十二輪重點（主機板：上方規格 facet）
- **側欄不變**：`腳位 > 晶片組 > 板廠`（不把板型/DDR 塞回樹）。
- **工具列 facet**（僅 category=motherboard 顯示，比照螢幕）：板型／記憶體槽／Wi-Fi／記憶體世代／有線網。
- **specs 五欄**：`mbForm` `mbDimm` `mbWifi` `mbDdr` `mbLan`；API `mb_form`…`mb_lan`。
- **規則重點**：Mini-ITX→2 槽；短品名缺 Wi-Fi 訊號→未標示；詳列 LAN+相供電且無無線→無 Wi-Fi；LAN **2.5 先於 5**。
- **DDR 判定**：平台定案（AM5/LGA1851→DDR5、AM4/LGA1200→DDR4）。**僅 LGA1700（12–14 代）雙軌**：品名 `D4`/`DDR4`→DDR4；`D5`/`DDR5`→DDR5；COMBO 雙支援→`DDR4/DDR5`；靜默 DDR4 白名單（B760M H、H610M-H2…）；其餘未標 D4 → **預設 DDR5**（台系慣例 DDR4 必標）。**mbDdr 未標示 = 0**。
- **覆蓋（842 件）**：DDR5 ~714／DDR4 ~127／DDR4/DDR5 少數；板型幾乎全有；插槽明示少（未標居多，預期）。

### 第三十一輪重點（螢幕：消減可回收「未標示」）
- **L0**：像素／U-Q 型號／49·57 帶魚／34 超寬；**15–25 吋**無高解析標記→FHD；**27 吋辦公 PRO/VA/GW…**→FHD（非全 27 一律）。
- **L1**：`monitor-catalog.json` + model key，merge **只填未標示**。
- **污染**：Trofeo Vision LCD 磁吸小屏→OTHER（不再 86 吋未標示）。
- **結果**（比價組視角）：螢幕 **1,191 組**；res 未標示 **20**；panel 69；Hz 427。
- **補洞**：ASUS `…AQ…` 中綴（**含 VA27AQ**）、MAG `CQ`、AOC `Q27`、Alienware AW、catalog 最長前綴近接。
- **誠實殘留**：Google TV、雙模 GX790、品名無解析度訊號者；Hz/panel 未標示仍允許。
...
- **螢幕 enrichment（已做 M0+M1）**：L0 品名 → L1 `monitor-catalog.json`；L2 外查未做。`merge` 不覆寫品名已抽值。
...
- **數量一致**：頂欄／側欄／子分類／品牌／列表一律 `match_groups`（「組」）。`getCategories/getSubcategories/getBrands/getMatchGroupCount`；頂欄不可再加總來源 `productCount`。
