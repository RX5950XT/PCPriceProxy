...
## 22. 件數 vs 組數：UI 所有數量一律比價組
- **問題**：列表／側欄用 `match_groups`，頂欄卻加總來源 `productCount`（`products` 列）。螢幕 1,271 件 vs 側欄 1,191 組，使用者以為系統算錯。
- **規則**：**凡使用者可見的數量**（頂欄總量、側欄分類／子分類／品牌、列表「共 N 組」）**全部查 `match_groups`**。`products` 列數與來源 `productCount` 只給 `/health` 與爬取監控，不可進 Dashboard 總量。
...
## 43. 「無中生有」的資料多半是他品類詞誤中關鍵字 / 料號子字串誤判
...
## 52. 主機板規格走工具列 facet，不塞回側欄樹
- **問題**：側欄只有腳位/晶片組/品牌，使用者要板型、插槽、Wi-Fi 等篩選，但塞進 path 會讓樹爆炸且與「腳位優先」衝突。
- **規則**：比照螢幕——側欄維持 `腳位 > 晶片組 > 板廠`；板型/DIMM/Wi-Fi/DDR/LAN 寫 `specs.mb*`，Dashboard 上方 facet + API query。短品名缺訊寫「未標示」，不猜；LAN **2.5 必須先於 5**（`2.5Gb` 含子字串 `5Gb`）。
- **套用**：改 specs 後跑 `clean-and-rebuild`；UI 計數仍用 match_groups。
