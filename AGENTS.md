...
- **UI 計數單位＝`match_groups`（比價組）**：頂欄總量、側欄分類／子分類／品牌、列表「共 N 組」必須同源；禁止用 `products` 列數或來源 `productCount` 當 Dashboard 總量（跨店同 SKU 會灌水，例螢幕 1271 件 ≠ 1191 組）。
...
- **主機板＝側欄 `CPU 腳位 > 晶片組 > 板廠` + 工具列 facet 板型／記憶體槽／Wi-Fi／DDR／有線網**（正交、不進 path）。`detectMotherboardSubcategory` 先抓晶片組，用 `CHIPSET_SOCKET` 映射到腳位（新增晶片組要同步補這張表與 `CHIPSET_ORDER`）；晶片組抓不到才退品名 socket token。**規格不進側欄樹**。`motherboardSpecFields` 寫 `mbForm`／`mbDimm`／`mbWifi`／`mbDdr`／`mbLan`。**DDR**：平台定案（AM5/LGA1851→DDR5、AM4/LGA1200→DDR4）；**僅 LGA1700 雙軌**——品名 `D4`→DDR4、`D5`→DDR5、COMBO→`DDR4/DDR5`、靜默 DDR4 白名單、其餘未標 D4→**預設 DDR5**（台系 DDR4 必標）。API：`mb_form`／`mb_dimm`／`mb_wifi`／`mb_ddr`／`mb_lan`。LAN **2.5 先於 5**。
...
- **機殼子分類＝`最大板型 > 品牌 > 系列`**（DIY 先看主機板相容）：`Mini-ITX` / `M-ATX` / `ATX` / `E-ATX` / `機架式 / 工業` / `未標板型`；判定序 工業機架→E-ATX→M-ATX→Mini-ITX→ATX（E-/M- 必須早於裸 ATX）→`CASE_FORM_BY_SERIES` 系列回填（AP201/AP202/MOTI Mini/C5 Curve…）→未標。`CASE_SERIES` 為**品牌範圍**表；側欄板型排序用 `exactTopRank`（不可 `includes('ATX')` 誤中 E-ATX）。配件（支撐架/燈條/USB 模組/HUB/垂直顯卡支架/機殼專用/↪）與 AIO 相容說明含「機殼」字樣走 `isCaseContaminated`；機殼+電源（含 `Focus GX-850` 瓦數藏型號）/機殼+散熱促銷走 PACKAGE，不可把「內含 SFX 850W+水冷」當 A+B。
- **螢幕＝側欄樹 `尺寸[> 實際吋／未標吋數] > 品牌` + 工具列 facet 面板／更新率／解析度**（正交、不進 path）。**三欄 specs 必填**：偵測不到寫 `未標示`。面板含 `量子點`。解析度 FHD／2K／4K／超寬／帶魚…。API `panel`／`refresh_tier`／`resolution`。型號回填吋。支架／筆電架／直播控制器／↪／訊號線走 `isMonitorContaminated`。
- **螢幕規格分層（L0→L1）**：L0=`detectMonitor*`（吋／面板／Hz／解析度；15–25 吋無高解析標記→FHD；27 吋辦公線→FHD；磁吸小 LCD／Trofeo Vision LCD 走 `isMonitorContaminated`）；L1=`monitor-catalog.json` + `mergeMonitorSpecs` **只填未標示**，不可覆寫品名已抽值。禁止「全庫沒寫就 FHD」。`npm run enrich:monitor-seed` 列缺口。L2 外查未實作。
- **漏分類 vs 雜訊**：機殼誤入 MB/CPU→`looksLikeCase` 重判 CASE（支撐架配件不可誤殺真機殼「內附支撐架」）。風扇純控制器/HUB/線→`isFanContaminated`。網通：軌跡球/↪/純 IoT 閘道污染；類型補交換器（N 埠/SFP）、Wi-Fi 延伸器、AP、USB/PCIe 網卡。**散熱（COOLER）**＝`一體式水冷 (AIO) > 冷排 mm > 燈效` / `單塔|雙塔|下吹空冷 > 高度階 > 燈效` / `散熱膏/配件 > M.2 散熱|分體水冷配件`。`isAioCooler`：MasterLiquid/CoreLiquid/LC/HydroShift/Panorama/Frozen/冷頭/裸排/預裝風扇+240–420（**不可**只靠「一體式風扇」＝系統扇）。空冷高度**只信「高／高度 N」**（禁止裸 `360mm` 當塔高）。分體件（水冷液/G1/4/接頭/水箱/SWAFAN）→`分體水冷配件`（須在 AIO 前判）。筆電散熱墊 NotePal→`isCoolerContaminated` 不入庫。PSU 缺瓦數→`未標瓦數`。CPU：Xeon 工作站桶；舊款→`其他／舊款`。品牌別名：海韻/台達/利民/佰維/UMAX/Glorious/Ktnet。
