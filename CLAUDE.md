...
- **組合 vs 條件價單品**：`isRealBundle`（真組合/整機/筆電：A+B 接真零件、CPU+GPU+RAM+儲存完整主機、欣亞PC/電競電腦等通路整機、周邊套裝、系統關鍵字）才歸 PACKAGE；`detectPriceCondition`（搭板/限組裝/套裝搭購…）只記 `specs.priceCondition`、單品仍歸真分類。A+B 比對**務必先中和假加號**（相電源/clearance/PSU認證 80+~92+/容量加號），否則單品被自身規格誤判成組合。`matcher` 會排除帶 `priceCondition` 者的跨店比價。
