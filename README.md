...
- **零件語意分類**：20 個主分類、依實際資料設計的多層子分類——主機板 `CPU 腳位 > 晶片組 > 板廠`（`AMD AM5 > B850 > MSI`）、顯卡 `系列 > 型號 > [多顯存才分容量] > 品牌`（`RTX 5090 > GIGABYTE`）、電源 `尺寸 > 瓦數 > 認證 > 模組`（`SFX-L 電源 > 750W~1000W > 80+ 白金牌`）、記憶體含「伺服器記憶體」層、機殼 `品牌 > 系列`（`Lian Li > O11`）、鍵鼠/耳機/喇叭/網通 `品牌 > 類型`（`Logitech > 無線`）；依晶片組世代 / GPU 世代 / 容量做語意排序。
- **資料清洗**：整機/筆電/掌機/工作站歸「整機/組合」；搭板價、限組裝、加購優惠等條件價單品標記後排除跨店比價；電競椅、印表機、電源擴充線、硬碟外接盒、AIO 水冷誤入 CPU、伺服器 RDIMM 誤入桌上型等污染自動修正/過濾。
