...
- **零件語意分類**：19 個主分類、依實際資料設計的多層子分類——主機板 `CPU 腳位 > 晶片組 > 板廠`（`AMD AM5 > B850 > MSI`）、顯卡 `系列 > 型號 > [多顯存才分容量] > 品牌`（`RTX 5090 > GIGABYTE`）、電源 `尺寸 > 瓦數 > 認證 > 模組`（`SFX-L 電源 > 750W~1000W > 80+ 白金牌`）、記憶體含「伺服器記憶體」層、機殼 `最大板型 > 品牌 > 系列`（`E-ATX > Lian Li > O11`）、鍵盤 `機制 > 軸 > 有線/無線 > 品牌`、滑鼠 `用途 > 有線/無線 > 品牌`、耳機/麥克風 `產品大類 > 品牌`、網通 `設備類型 > 品牌`、線材 `大類 > 細類`（`影音線 > HDMI`）、喇叭 `品牌 > 類型`、線材 `類型`（`影音線`・`機內排線 / 延長線`）、作業系統 / 軟體 `作業系統 > Windows 11`・`應用軟體 > 防毒軟體`、整機/組合 `整機電腦・伺服器 / 工作站・筆電・準系統・掌機・零件組合・搭購價單品`；依晶片組世代 / GPU 世代 / 容量做語意排序。
